...
class SmartCartApp(tk.Frame):
    ...
    def _poll_weight(self):
        try:
            ...
            self.weight_display.config(text=f"⚖ {g:.1f} g")
        except Exception:
            pass
    ...
    def _on_popup_confirmed(self, product_data, quantity: int):
        ...
        expected_g = self.weight_validator.unit_to_grams(qty_value, qty_unit) * quantity

        # Fresh baseline — read right now while platform is still empty
        self._baseline_weight    = self.weight_validator.scale.read_grams()
        self._weight_wait_active = True

        self.update_status(
            f"📦 Place {quantity}× {name} on scale — expected {expected_g:.0f}g", "info")

        self.after(int(WEIGHT_STABLE_DELAY * 1000),
                   self._wait_for_weight_match,
    ...
    def _wait_for_weight_match(self, product_data, quantity, expected_g,
        ...
        if not self._weight_wait_active:
            return   # cancelled externally (tare / checkout)

        barcode, name, price, discount, qty_value, qty_unit = product_data

        current  = self.weight_validator.scale.read_grams()
        delta    = current - baseline
        elapsed  = time.time() - t0
        remaining = int(WEIGHT_WAIT_TIMEOUT - elapsed)
        tolerance = max(MIN_TOLERANCE_GRAMS, expected_g * TOLERANCE_PERCENT / 100)
        diff      = abs(delta - expected_g)

        # Live status update
        self.update_status(
            f"⚖ {name}  Live: {delta:.0f}g / Expected: {expected_g:.0f}g"
            f"  ({remaining}s)", "info")

        if delta >= 5 and diff <= tolerance:
            # ✅ Weight matched → add to cart
            self._weight_wait_active = False
            self.add_item(barcode, name, price, discount, qty_value, qty_unit, quantity)
            self.update_status(
                f"✓ Added {quantity}× {name}  ({delta:.0f}g detected)", "success")
            return

        if elapsed >= WEIGHT_WAIT_TIMEOUT:
            # ⏰ Timeout → staff override popup
            self._weight_wait_active = False
            result = {
                "valid":    False,
                "expected": expected_g,
                ...
                "message":  (f"Timeout: expected ~{expected_g:.0f}g, "
                             f"detected {delta:.0f}g. "
